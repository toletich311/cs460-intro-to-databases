...
public class TableIterator {
    ...
    public Object getColumnVal(int colIndex) {
        ...
        int start = valueIn.readShortAtOffset(colIndex*2);
        ...
        if (start==-1){
            ...
            return keyIn.readBytesAtOffset(0,length); 
        }

        int length = col.getLength();

        if (col.getType()==0){
            return valueIn.readIntAtOffset(start);
        } else if (col.getType()==1){
            return valueIn.readDoubleAtOffset(start);
        } else if (col.getType()==2){
            return valueIn.readBytesAtOffset(start, length);
        } else{
            int end; 
            do{
            ...
            return valueIn.readBytesAtOffset(start, end-start);
        }
    }
